    ...
    try {
     ...
     // if (storedCsrfToken) {
       ...
       // setCsrfToken(storedCsrfToken);
        ...
        const response = await fetch('http://localhost/api/client/loginE', {
          method: 'POST',
          headers: {
        ...
        if (userData.responseCode === "Login Success") {
          localStorage.setItem('token', userData.responseMessage);
          navigate('/Employe'); // Redirect to profile page on successful login
        } else {
